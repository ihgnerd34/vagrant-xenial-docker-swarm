...
This is a simple Vagrantfile which can be used to spin few nodes with Docker-ce installed. You
...
By default, vagrant will initialize the swarm. To only create pure machines with docker installed, you can run 
`AUTO_START_SWARM=false vagrant up` to avoid provisioning the swarm automatically.
...
Go to the master node and display the docker swarm:
...
Updated to Xenial, the latest stable docker-ce, and enabled the auto swarm mode by default
